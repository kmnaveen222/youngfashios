...
@Component({
  selector: 'app-mensadmin',
  templateUrl: './mensadmin.component.html',
  styleUrls: ['./mensadmin.component.css']
})
export class MensadminComponent {
  pro:any;
  itemid:any;
...
addproduct=this.forms.group({
  ...
  Department:['mens',[Validators.required]],
...
getproduct(){
  ...
  onEdit(item:any){
   this.itemid=item.id;
    ...
    this.addproduct.controls['sizeS'].setValue(item.sizeS);
    this.addproduct.controls['sizeM'].setValue(item.sizeM);
    this.addproduct.controls['sizeL'].setValue(item.sizeL);
    this.addproduct.controls['sizeXL'].setValue(item.sizeXL);
    this.addproduct.controls['sizeXLL'].setValue(item.sizeXLL);
...
update(){
  if(this.addproduct.valid){
this.http.put<any>(this.url+'/mens-product/'+`${this.itemid}`,this.addproduct.value).subscribe(res=>{
...
ondelete(item:any){
  this.http.delete(this.url+'/mens-product/'+`${item.id}`).subscribe((res =>{
