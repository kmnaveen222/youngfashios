...
@Component({
  selector: 'app-kidsadmin',
  templateUrl: './kidsadmin.component.html',
  styleUrls: ['./kidsadmin.component.css']
})
export class KidsadminComponent {
...
getproduct(){
 ...
 update(){
   if(this.addproduct.valid){
 this.http.put<any>(this.url+'/kids-product/'+`${this.itemid}`,this.addproduct.value).subscribe(res=>{
 ...
 ondelete(item:any){
   this.http.delete(this.url+'/kids-product/'+`${item.id}`).subscribe((res =>{
